...
python run.py \
  ...
  --input_vsize 150000 \
  --ckpt_root=checkpoints/cnndaily/seqmatch/ex08 \
  --summary_dir=log/cnndaily/seqmatch/ex08 \
  --mode=train \
  --lr 0.2 \
  --min_lr 0.01 \
  --max_grad_norm 2.0 \
  ...
  --sm_conv1d_width 3 \
  --sm_conv_filters "256,512,1024" \
  --sm_conv_heights "3,3,3" \
  --sm_conv_widths "3,3,3" \
  --sm_maxpool_widths "2,2,2" \
