...
class SummaRuNNer(object):
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope("seq2seq"):
      with tf.variable_scope("embeddings"):
        self._add_embeddings(input_vsize)
  ...
  def _add_embeddings(self, input_vsize):
    hps = self._hps
  ...
  def run_train_step(self, sess, batch):
    enc_batch, enc_doc_lens, enc_sent_lens, sent_rel_pos, extract_targets, target_weights = batch
  ...
  def get_extract_probs(self, sess, batch):
    enc_batch, enc_doc_lens, enc_sent_lens, sent_rel_pos, _, _ = batch
